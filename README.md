...
> **NOTE**: This developer pattern creates a blockchain network on *IBM Blockchain Platform version **2.5*** using the *Hyperledger Fabric version **2.2***.
...
* Press the `F1` key to see the different VS code options. Choose `IBM Blockchain Platform: Package Open Project` and pick the `cds (V1 channel capabilities)` output format.
...
### 5. Deploy Blockchain for maintaining Digital Assets Smart Contract on the network
